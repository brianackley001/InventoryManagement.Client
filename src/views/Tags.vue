    ...
    <v-row v-show="!loading && pagedCollection.collectionTotal == 0">
      <v-col>
        <v-alert border="left" colored-border type="warning" elevation="2">
          There are no existing tags... please create a new group in order to
          attribute a tag to any of the items in your inventory.
        </v-alert>
      </v-col>
    </v-row>
    <v-row>
      <v-col cols="10">
        <div class="subtitle">Tags ({{ pagedCollection.collectionTotal }})</div>
      </v-col>
      <v-col cols="2">
        <SortMenu
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("tags", ["items", "pageSize", "pagedCollection"]),
    currentPage: {
      get: function () {
  ...
  methods: {
    ...mapActions("tags", ["fetchTagsWithItemCounts", "setCurrentPage"]),
    ...
    pageInput(value) {
      ...
      this.fetchTagsWithItemCounts(this.searchRequest);
    },
  ...
  created() {
    this.$eventHub.$on(
      "on-upsert-tag-attribute-completed",
      this.refreshPagedCollection
    );
  },
  beforeDestroy() {
    this.$eventHub.$off("on-upsert-tag-attribute-completed");
  },
};
</script>
